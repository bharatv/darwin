...
echo "Starting darwin-ofs-v2-writer..."
...
JAVA_OPTS="${JAVA_OPTS} -XX:HeapDumpPath=/opt/logs/darwin-ofs-v2-writer-heapdump.hprof"
...
case "${ODIN_DEPLOYMENT_TYPE}" in
  *container*|*k8s*)
    ...
    ;;
  *)
    nohup java ${JAVA_OPTS} -jar "${ODIN_APP_DIR}/${ARTIFACT}" > /opt/logs/darwin-ofs-v2-writer.log 2>&1 &
