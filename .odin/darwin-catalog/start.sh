...
JAVA_OPTS="${JAVA_OPTS} -XX:HeapDumpPath=/opt/logs/darwin-catalog-heapdump.hprof"
...
case "${ODIN_DEPLOYMENT_TYPE}" in
  *container*|*k8s*)
    ...
    ;;
  *)
    nohup java ${JAVA_OPTS} -jar "${ODIN_APP_DIR}/${ARTIFACT}" > /opt/logs/darwin-catalog.log 2>&1 &
