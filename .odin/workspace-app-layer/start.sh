...
echo "Starting workspace-app-layer..."
...
case "${ODIN_DEPLOYMENT_TYPE}" in
  *container*|*k8s*)
    exec uvicorn workspace_app_layer.main:app --host 0.0.0.0 --port 8080
    ;;
  *)
    nohup uvicorn workspace_app_layer.main:app --host 0.0.0.0 --port 8080 > /opt/logs/workspace-app-layer.log 2>&1 &
