# =============================================================================
# EKS MODULE - Managed Kubernetes for Darwin Platform
# =============================================================================
...
# =============================================================================
# COMPUTE NODE GROUP (Optional - for ML workloads)
# =============================================================================
...
# =============================================================================
# S3 ACCESS IAM ROLE FOR PODS (IRSA)
# =============================================================================

resource "aws_iam_role" "s3_access" {
  name = "${local.cluster_name}-s3-access-role"
    ...
    Statement = [
      {
        ...
        Condition = {
          StringLike = {
            "${replace(aws_iam_openid_connect_provider.cluster.url, "https://", "")}:sub" = "system:serviceaccount:darwin:*"
            "${replace(aws_iam_openid_connect_provider.cluster.url, "https://", "")}:aud" = "sts.amazonaws.com"
          }
        }
      }
...
resource "aws_iam_role_policy" "s3_access" {
  name = "${local.cluster_name}-s3-access-policy"
...
# =============================================================================
# SECRETS MANAGER ACCESS IAM ROLE FOR PODS (IRSA)
# =============================================================================

resource "aws_iam_role" "secrets_access" {
  name = "${local.cluster_name}-secrets-access-role"
...
resource "aws_iam_role_policy" "secrets_access" {
  name = "${local.cluster_name}-secrets-access-policy"
